...
slope_angle = 40 / 180 * pi # in radians
weight_edge_slope = 10.0
...
add_curvature_constraint = False
crv_min = -100.0
crv_max = -0.1
...
if add_edge_slope_goal:
    for u, v in mesh.edges():
        edge = None
        if mesh.is_vertex_on_boundary(u) and not mesh.is_vertex_on_boundary(v):
            edge = (u, v)
        elif not mesh.is_vertex_on_boundary(u) and mesh.is_vertex_on_boundary(v):
            edge = (v, u)
        if edge is not None:
            print(edge)
            goal = EdgeAngleGoal(edge, [0.0, 0.0, 1.0], slope_angle, weight=weight_edge_slope)
            goals.append(goal)
